...
int main() {
    ...
    for(int k=0;k<35;k++){
        ...
        for(int i=0;i>=0;i++){
//            if(!MultiGraphEnabled) Sleep(100);
//            if(i>=1000) Sleep(200);
            Recession(i);
...
void Initialize(double nodeCnt){
    ...
    for(int i=0;i<nodeCnt;i++) Graph.nodes.push_back(newNode),Graph.nodes[i].affiliated=0;
...
void CalcRepel(){
    for(int i=0;i<nodeCnt;i++){
        ...
        for(int j=i+1;j<nodeCnt;j++){
            ...
            // double FRepel=kRepel/Dist(x0,y0,x1,y1)*(Graph.nodes[i].weight+Graph.nodes[j].weight)/2;
...
//    cout<<"Overall="<<OverallEnergy<<" Limit="<<nodeCnt*minEnergy<<endl;
//    Debug(0);
