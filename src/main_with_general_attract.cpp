...
void Initialize(double nodeCnt){
    ...
    for(int i=0;i<nodeCnt;i++){
        for(int j=0;j<nodeCnt;j++){
            if(i==j) Graph.nodes[i].attraction_amplifier.push_back(-1);
            else Graph.nodes[i].attraction_amplifier.push_back(dist[i][j]);
        }
...
void CalcAttract(){
    for(int i=0;i<nodeCnt;i++){
        for(int j=i+1;j<nodeCnt;j++){
             double kAmplifier=pow(2.0,(-1.0)*(Graph.nodes[i].attraction_amplifier[j]-1));
...
void Recession(double round){
    const double startRecession=2;
    const double limRound=2500.0;
